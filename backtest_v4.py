"""v4 backtest: same offense-EPA + prior-blend + rest + div-shrink model as v3's
best variant, plus a referee-crew ATS-lean factor, to measure whether folding in
referee history actually moves the needle on ATS win rate.

Referee "Home ATS%" is computed as a running/expanding stat from ALL real games
each referee worked before the game being scored (starting from the earliest
season nflverse has referee data, 1999) -- never using games that hadn't
happened yet, same look-ahead-safe approach as the team ratings. Only applied
once a referee has 10+ prior games, matching the live app's threshold.
...
REFEREE_HISTORY_FROM = 1999  # earliest season nflverse's games.csv has referee names for
...
def load_schedule_all():
    ...
    for row in csv.DictReader(io.StringIO(text)):
        ...
        if row["game_type"] != "REG" or season < REFEREE_HISTORY_FROM:
            continue
        if not row["home_score"] or not row["away_score"]:
            continue
        by_season.setdefault(season, []).append({
            "season": season, "week": int(row["week"]),
            "home": row["home_team"], "away": row["away_team"],
            "home_score": int(row["home_score"]), "away_score": int(row["away_score"]),
            "spread_line": float(row["spread_line"]) if row["spread_line"] else None,
            ...
            "referee": (row["referee"] or "").strip() or None,
        })
    for s in by_season:
...
def build_referee_history(by_season):
    """Chronological (season, week, home_covered) log per referee, used to compute
    each referee's expanding-window Home ATS% as of any later game -- never peeking
    at games that haven't happened yet relative to the game being scored."""
    log = {}
    for season in sorted(by_season):
        for g in by_season[season]:
            if not g["referee"] or g["spread_line"] is None:
                continue
            home_margin = g["home_score"] - g["away_score"]
            result = home_margin - g["spread_line"]
            if result == 0:
                continue  # push -- doesn't count toward ATS%, same as live app's cover math
            log.setdefault(g["referee"], []).append((season, g["week"], result > 0))
    for ref in log:
        log[ref].sort(key=lambda t: (t[0], t[1]))
    return log


def referee_factor(ref_history, referee, season, week):
    if not referee:
        return 0.0
    entries = ref_history.get(referee, [])
    prior = [covered for (s, wk, covered) in entries if (s, wk) < (season, week)]
    if len(prior) < REF_MIN_GAMES:
        return 0.0
    home_ats_pct = sum(prior) / len(prior) * 100.0
    return clamp((home_ats_pct - 50.0) / 50.0 * 0.8, -REF_FACTOR_CAP, REF_FACTOR_CAP)


def run_season(season, all_schedules, all_stats, ref_history, detail_rows, use_referee):
    ...
    skipped_no_rating = skipped_no_spread = skipped_pass = 0
    ...
    for g in games:
        ...
        if use_referee:
            rating_margin += referee_factor(ref_history, g["referee"], season, g["week"])
...
def run_variant(name, use_referee, all_schedules, all_stats, ref_history):
    detail = []
    tiers_by_season = []
    for season in SEASONS:
        _, tiers, _, _ = run_season(season, all_schedules, all_stats, ref_history, detail, use_referee)
        tiers_by_season.append(tiers)
    w, l, p, pct = combined(tiers_by_season)
    print(f"{name:<45} n={w+l+p:<5} W-L-P {w}-{l}-{p:<4} ATS%={pct:.1f}%")

    print("  by season:")
    for season, tiers in zip(SEASONS, tiers_by_season):
        sw, sl, sp = 0, 0, 0
        for tier in ("LEAN", "LARGE LEAN"):
            sw += tiers[tier].count("WON"); sl += tiers[tier].count("LOST"); sp += tiers[tier].count("PUSH")
        sd = sw + sl
        spct = sw / sd * 100 if sd else float("nan")
        print(f"    {season}  W-L-P {sw}-{sl}-{sp:<4} ATS%={spct:.1f}%")
    return pct
...
def main():
    ...
    ref_history = build_referee_history(all_schedules)

    print(f"Referee sample: {len(ref_history)} distinct referees logged since {REFEREE_HISTORY_FROM}\n")

    baseline_pct = run_variant("baseline (prior blend + rest + div-shrink)", False, all_schedules, all_stats, ref_history)
    print()
    referee_pct = run_variant("+ referee crew ATS-lean factor", True, all_schedules, all_stats, ref_history)

    print(f"\nDelta from adding referee factor: {referee_pct - baseline_pct:+.2f} pts ATS%")
